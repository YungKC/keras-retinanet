...
def create_models(num_classes, weights='imagenet', multi_gpu=0, snapshot_start=None):
...
def create_generators(args):
    ...
    train_image_data_generator = keras.preprocessing.image.ImageDataGenerator(
        horizontal_flip=False,  # Kai - do not flip image for our case!
...
def parse_args():
    ...
    parser.add_argument('--snapshot_start', help='Name of snapshot to start with', type=str, default=None)
    parser.add_argument('--initial_epoch', help='Epoch number to start with', default=0)
...
if __name__ == '__main__':
    ...
    if args.snapshot_start != None:
        print('Initial_epoch: {}, Snapshot Used: {}'.format(args.initial_epoch, args.snapshot_path+"/"+args.snapshot_start))
    ...
    startSnapshot = None
    if args.snapshot_start != None:
        startSnapshot = keras.models.load_model(args.snapshot_path+"/"+args.snapshot_start, custom_objects=custom_objects)

    model, training_model, prediction_model = create_models(num_classes=train_generator.num_classes(), weights=args.weights, multi_gpu=args.multi_gpu, snapshot_start=startSnapshot)
